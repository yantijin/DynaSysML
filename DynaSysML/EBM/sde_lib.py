...
class SDE(abc.ABC):
    ...
    def discretize(self, x, t, *args, **kwargs):
        ...
        """
        dt = 1 / self.N
        drift, diffusion = self.sde(x, t, **kwargs)
        f = drift * dt
        G = diffusion * torch.sqrt(torch.tensor(dt, device=t.device))
        return f, G

    def get_score_par(self, t, *args, **kwargs):
        res = (t,)
        if args:
            res += args
        if kwargs:
    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        score_fn_par = self.get_score_par
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, t, *args, **kwargs):
                ...
                drift, diffusion = sde_fn(x, t, *args, **kwargs)
                denoise_par = score_fn_par(t, *args, **kwargs)
                score = score_fn(x, *denoise_par)
            ...
            def discretize(self, x, t, *args, **kwargs):
                ...
                rev_f = f - G[:, None, None, None] ** 2 * \
                    score_fn(x, *denoise_par) * (0.5 if self.probability_flow else 1.)
...
class VPSDE(SDE):
    def __init__(self, beta_min=0.1, beta_max=20, N=1000):
        """Construct a Variance Preserving SDE.(Continuous denoising diffusion probabilistic models)
