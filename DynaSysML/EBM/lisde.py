...
class LISDE(nn.Module):
    ...
    def rev_sampling(self, x, num_steps, denoise=True, clamp=True, *args, **kwargs):
        bs = x.shape[0]
        step = 1. / num_steps
        time_steps = torch.tensor([step]).repeat(num_steps)
        if denoise:
        ...
        for i, time_step in enumerate(time_steps):
            ...
            if clamp:
                x.clamp_(-2., 2.) # 这里需要调整
            # print(torch.isnan(x).any(), torch.max(x))
    ...
    def sample(self, shape, num_steps, device, denoise=True, clamp=True, *args, **kwargs):
        ...
        # x = self.euler_sampling(x, num_steps, denoise=denoise, clamp=clamp, *args, **kwargs)
        x = self.rev_sampling(x, num_steps, denoise=denoise, clamp=clamp, *args, **kwargs)
        return x
